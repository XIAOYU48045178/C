...
`指针数组: 指针数组是一个数组 数组元素是指针可以指向不同的内存位置` `数组指针: 数组指针是指针 指向整个数组 注意它的目标是整个数组而不是普通的指针那样指向的是数组的首个元素` `是 …… 看 after `

`! 整型数组指针`

```c
int arr[3] = {1, 2, 3};
int (*p)[3] = &arr; -- 组指针代表的是整个数组的地址 虽然和普通指针都是指向首元素地址 但是意义不同
-- *(*p+0) p[0][0] 1 *(*p+1) 2 *(*p+2) 3
```
...
int (*p)[3] = arr; 二维数组不需要再取地址了 因为维度提升 数组指针指向的是二维数组中的其中一个元素 元素本身就是一个数组
-- *(*(p+1)+2) 6 p[1][2] 6
```
...
`? 值传递 引用传递 ` `无论如何 传递数据类型需保持一致 或者 指针指向地址`
...
`? 传递数组给函数` `无论如何 传递数据类型需保持一致 或者 指针指向地址`
...
`一个指针` `int function(int *arr)` `-- 指针指向地址`

`一个已定义大小的数组` `int function(int arr[10])` `-- 相同类型`

`一个未定义大小的数组` `int function(int arr[])` `-- 相同类型`
...
`int function(int * arr[])` `-- 相同类型`

`int function(int ** arr)` `-- 指针指向地址`
...
`函数显示您传给它的字符串跟一个冒号一个空格和当前 errno 值的文本表示形式` 

`? strerror`

`函数返回一个指针 指针指向当前 errno 值的文本表示形式 有一点需要注意 您应该使用 stderr 文件流来输出所有的错误`
