...
```c
int b[3][3]={ {1,2,3},{4,5,6},7,8 }; -- b[2][1] = 8

int b[4][4]={ {1,2,3},{4,5,6},{7,8,9} }; -- b[0][2], b[1][3], b[2][1], b[3][0] 3 0 8 0
...
`结构是一种自定义的可用的数据类型 允许您存储不同类型的数据项` `结构体中的数据成员可以是基本数据类型 也可以是其他结构体类型 指针类型 `
...
`结构体指针
 ...
 .          运算得到结构体变量中的成员对象本身 | 结构体变量 
 ->         运算得到结构体变量中的成员对象本身 | 结构体指针 
...
这是为了优化结构体在内存中的存储,使得CPU可以高效地访问结构体的每个成员。



当CPU访问结构体中的成员时,只需要做一次内存访问即可读到该成员全部内容。否则,它需要做两次或更多次内存访问来读到整个成员的内容,这降低了程序的执行效率。





1.存放成员的起始地址必须为此成员的有效对齐值的整数倍
2.存放结构体的起始地址必须为此结构体有效对齐值的整数倍
...
填充字节(padding)指的是结构体或类成员之间用于填充以实现内存对齐的额外字节。

结构体或类在内存中的布局需要遵循对齐原则,使得每个成员变量的起始地址都是其类型大小的整数倍。为了满足这个要求,编译器会在成员之间插入一些没有实际意义的填充字节。
