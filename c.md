...
```c
int arr1[2] = {1, 2};
int arr2[2][2] = { {1, 2}, arr1[0]}; -- i i + 1 j + 1 k + 1 000000974A7FF990 000000974A7FF998 000000974A7FF994 000000974A7FF994
...
你不能直接将char *args[] 换成一个二维字符数组，因为它们的类型和意义是不同的。char *args[] 是一个字符指针数组，每个元素都是一个指向 char 的指针（即字符串）。而二维字符数组是一个数组的数组，每个元素本身又是一个字符数组。

如果你想要用一个二维数组来存储多个字符串，你需要指定每个字符串的最大长度，并且通常你需要自己处理字符串的结尾（比如确保每个字符串都以空字符\0 结尾）。
