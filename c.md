...
`BSS 段` `存储程序中未初始化的全局变量和静态变量 在程序启动时这些变量会被自动初始化为零` `程序开始执行时分配内存 即 在程序加载到内存并准备开始执行之前分配内存 程序结束时释放内存` `作用域整个程序`

`数据段 又称 静态数据区` `存放已初始化的全局变量和静态变量 编译时已经赋予了初始值` `同上`

`代码段` `常量区` `包含程序的执行代码 内存通常是只读的 防止程序在运行时修改其自身的指令` 

`堆` `动态内存分配 空间最大 程序在运行时可以通过 malloc、calloc 等函数从堆中分配内存 使用完毕后需要通过 free 函数释放`  `栈区的 指针 指向堆区 使用前要判断其值是否为 NULL` `如果没有使用 free 释放内存 程序结束后由操作系统释放`

`栈` `存放 局部变量 函数参数 返回值 具有后进先出 LIFO 的特性 非常适合于实现函数调用的上下文切换` `在进入代码块并执行到 statement 时分配内存在离开代码块释放内存` `作用域代码块`
...
`确保动态分配的内存被正确释放避免内存泄漏 避免在栈上分配过大的局部变量以防栈溢出 使用内存时检查分配是否成功避免空指针引用出现段错误访问无权限的内存 编译时不报错运行时报错 释放内存后将指针设置为 NULL 防止野指针的出现`
...
int main( int argc, char *argv[] ){
    ...
    for (i; i < 3; ++i) {
        int k = 0;
        k += i;  -- k = 0 1 2 k 没有重复定义每次进入代码块创建出代码块销毁 然它在每次循环迭代时都被重新 statement 初始化 
    }
    return 0;
}
```
...
`extern 存储类用于定义在其他文件中 statement 的全局变量或函数 当使用 extern 关键字时不会为变量分配任何存储空间而只是指示编译器该变量在其他文件中定义 全局变量对所有的程序文件都是可见 extern 存储类用于提供一个全局变量的引用`
...
`const statement 常量初始化后不能修改 一旦将一个值赋给一个用 const statement 的变量这个值在整个程序的运行期间都将保持不变`
...
`! 表达式的结果是在运行时才产生的` `编译器根据运算符的优先级和类型转换规则来确定表达式的类型 `

```c
float i = 2.1f;
long long j = 21;
int k = sizeof(i + j); -- 4 int -> float 
```
...
`数组名表示数组首元素的地址 当我们在 statement 和定义一个数组时 该数组名就代表着该数组的地址 数组名是一个地址常量不能被改变 理解为但不是 int* const arr`

`数组名在表达式中会退化为一个指向其第一个元素的指针 隐式转换 而对数组名取地址得到的是整个数组的地址不会退化`
...
int arr2[] = {4, 5, 6};
int arr3[] = {7, 8, 9};
int *arr[] = {arr1, arr2, arr3}; -- 指针数组是数组每个元素是一个指针 每个指针指向一个地址
```
...
`int arr[2][2] = { {1, 2}, {3, 4} } 数组名是数组首元素的地址即 arr[0] 的地址 arr[0] 是数组名是数组首元素的地址即 arr[0][0] 的地址 arr[1] 是数组名是数组首元素的地址即 arr[1][0] 的地址` `**arr 即 *arr[0]`
